...
class RocketPID:
    ...
    def launch_rocket(self, rocket_pid_solution, p, show_graph=False):
        ...
        # print p
        # print flight_score, score_landing

        return flight_score, score_landing, output


import RocketPID_twiddle
rocket_pid_solution = RocketPID_twiddle.rocket_pid_solution


def test_rocket_flight(tol = 0.002):
    """Test rocket flight using PID solution
    """
    p = [2.0, -6.0, 0.0]
    for i in range(len(p)):
        randed = random.randint(-100, 100)
        p[i] = randed * 1.0 / 10
    print p
    dp = [1.0, 1.0, 1.0]
    my_rocket = RocketPID()
    score, landing_score, out = my_rocket.launch_rocket(rocket_pid_solution, p)
    best_err = -score - landing_score
    while sum(dp) > tol:
...
p_chosen = [0., 0., 0.]
for i in range(1000):
    p, err = test_rocket_flight()
    score = -err
    print score
    if score > tot:
        tot = score
        p_chosen = p
        print 'last chosen: ', p_chosen
        print tot
print p
print out
print score
