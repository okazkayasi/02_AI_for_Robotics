...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function   
        ...
        #execute
        beta = (d2/self.length) * tan(alpha2)
        the_robot = robot(self.length)
        the_robot.set_noise(self.bearing_noise, self.steering_noise, self.distance_noise)
        ...
        if beta != 0:          
            ...
            the_robot.x = cx + sin(self.orientation + beta) * R
            the_robot.y = cy - cos(self.orientation + beta) * R
            the_robot.orientation = (self.orientation+beta) % (2*pi)                       
        else:
            the_robot.x = self.x + d2 * cos(self.orientation)
            the_robot.y = self.y - d2 * sin(self.orientation)            
...
print ("x=93.476 y=75.186 orient=5.2664")
