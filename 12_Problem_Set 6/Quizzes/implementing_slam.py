...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    # start both matrices first
    # [0] is sense [landmark_index, dist_x, dist_y]
    # [1] is [dx, dy]
    matrix_size = N + num_landmarks
    print 'matrix size: ', matrix_size
    mot = 1./motion_noise
    mea = 1./measurement_noise

    add_x = matrix([[1.]])
    Omega_x = add_x.expand(matrix_size, matrix_size, [0], [0])
    Omega_y = add_x.expand(matrix_size, matrix_size, [0], [0])

    add_vec = matrix([[world_size/2.0]])
    Xi_x = add_vec.expand(matrix_size, 1, [0], [0])
    Xi_y = add_vec.expand(matrix_size, 1, [0], [0])
    print 'len data: ', len(data)
    for i in range(N-1):
        index = i + 1
        # index - 1 and index will be updated.
        # also the landmarks and index.
        sub_mat = matrix([[mot, -mot],
                          [-mot, mot]])
        sub_mat = sub_mat.expand(matrix_size, matrix_size, [index-1, index], [index-1, index])
        ...
        dx = data[i][1][0]
        dy = data[i][1][1]
        sub_vec_x = matrix([[-dx], [dx]])
        sub_vec_y = matrix([[-dy], [dy]])

        sub_vec_x = sub_vec_x.expand(matrix_size, 1, [index-1, index], [0])
        sub_vec_y = sub_vec_y.expand(matrix_size, 1, [index-1, index], [0])

        Xi_x += sub_vec_x
        Xi_y += sub_vec_y

        # check how many landmarks are seen
        for info in data[i][0]:
            # info [0] -> index of landmark
            # info [1] -> x dist
            # info [2] -> y dist
            # first index is index variable already
            # second index is N + 1 + landmark index
            land_index = N + info[0]
            dx = info[1]
            dy = info[2]
            sub_mat = matrix([[mea, -mea],
                              [-mea, mea]])
            
            sub_mat = sub_mat.expand(matrix_size, matrix_size, [index-1, land_index], [index-1, land_index])
            ...
            sub_vec_x = matrix([[-dx], [dx]])
            sub_vec_y = matrix([[-dy], [dy]])
            sub_vec_x = sub_vec_x.expand(matrix_size, 1, [index-1, land_index], [0])
            sub_vec_y = sub_vec_y.expand(matrix_size, 1, [index-1, land_index], [0])
    ...
    mu_x = Omega_x.inverse() * Xi_x
    mu_y = Omega_y.inverse() * Xi_y

    mu_x = mu_x.expand(matrix_size*2, 1, range(0, matrix_size*2, 2), [0])
    mu_y = mu_y.expand(matrix_size*2, 1, range(1, matrix_size*2, 2), [0])
