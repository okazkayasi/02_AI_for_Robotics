...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    # set the dimension. will keep all landmarks
    dim = 2 * (num_landmarks + 1)

    # make the constraint information matrix and vector (same as full slam)
    Omega = matrix()
    Omega.zero(dim, dim)
    Omega.value[0][0] = 1.0
    Omega.value[1][1] = 1.0
    ...
    for k in range(len(data)):
        ...
        # make Omega and Xi larger 
        indices = [0,1]
        indices.extend(range(4, dim+2))

        Omega = Omega.expand(dim+2, dim+2, indices, indices)
        Xi = Xi.expand(dim + 2, 1, indices, [0])
        ...
        # primes
        Omega_prime = Omega.take(range(2, dim+2), range(2, dim+2))
        Xi_prime = Xi.take(range(2, dim+2), [0])

        Omega = Omega_prime - A.transpose() * B.inverse() * A
        Xi = Xi_prime - A.transpose() * B.inverse() * C
