...
def next_move(hunter_position, hunter_heading, target_measurement, max_distance, OTHER = None):
    ...
    turning = 0
    distance = 0
    [x,y], OTHER = estimate_next_pos(target_measurement, OTHER)
    heading_to_target = get_heading(hunter_position, [x,y])
    heading_difference = heading_to_target - hunter_heading
    turning =  angle_trunc(heading_difference) # turn towards the target
    distance = distance_between(hunter_position, [x,y]) # full speed ahead!
