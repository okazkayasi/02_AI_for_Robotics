...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        # left center 
        left_center = [radius, radius]
        right_center = [radius * 3, radius]
        
        # robot is at the left circle
        if self.x <= radius:
            squared = (left_center[0]-self.x)**2 + (left_center[1]-self.y)**2
            return squared**0.5 - radius
        # robot is at straight part
        if self.x > radius and self.x < radius*3:
            # top part
            if self.y > radius:
                return self.y - radius * 2
            else:
                return -self.y
        # robot is at the right circle
        if self.x >= radius:
            squared = (right_center[0]-self.x)**2 + (right_center[1]-self.y)**2
            return squared ** 0.5 - radius
...
def run(params, radius, printflag = False):
    ...
    for i in range(N*2):
        ...
        if printflag:
            print myrobot
            print (degrees(myrobot.orientation) + 90) % 360
    return err / float(N)
